...
| Public ranges need a quick neutral classification. | Report whether DiskLoom's median is below, within, or above each public range without treating that as a same-machine proof. | `public-comparison.csv` includes `diskloom_median_position`. |
...
- Treat public rows with `comparison_applicability` other than `aligned_ntfs_mft` as progress references only, not scanner-class comparisons.
