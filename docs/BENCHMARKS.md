...
```powershell
target\release\diskloom-bench.exe suite C:\ target\bench-suite --dataset-label workstation-c --cache-state warm --iterations 5 --sample-ms 10 --progress-every 1024 --scanner ntfs --competitor-csv target\competitors.csv
...
.\scripts\run-bench-suite.ps1 -Path C:\ -Scanner ntfs -Iterations 5 -DatasetLabel workstation-c -CacheState warm -CompetitorCsv target\competitors.csv
